...
public class ThinnessClassFinderTest {
    @Test
    public void testName() throws Exception {
        ...
        ThinnessClassFinder uut = new ThinnessClassFinder();
        Set<ThinnessClass> result = uut.findFrom(dgraph);

        assertThat(result).hasSize(3);
        assertThat(result).anySatisfy(tc -> {
            assertThat(tc.contains(node1)).isTrue();
            assertThat(tc.getNodes()).hasSize(1);
        }).anySatisfy(tc -> {
            assertThat(tc.contains(node2)).isTrue();
            assertThat(tc.contains(node3)).isTrue();
            assertThat(tc.getNodes()).hasSize(2);
        }).anySatisfy(tc -> {
            assertThat(tc.contains(node4)).isTrue();
            assertThat(tc.getNodes()).hasSize(1);
        });
    ...
    public void testName2() throws Exception {
        ...
        DiGraph dgraph = new DiGraph(Sets.newHashSet(node1, node2, node3, node4, node5, node6,
                node7, node8),
                Sets.newHashSet(edge1, edge2, edge3, edge4, edge5, edge6, edge7, edge8, edge9,
        ...
        assertThat(result).anySatisfy(tc -> {
            ...
            assertThat(tc.contains(node3)).isTrue();
            assertThat(tc.contains(node4)).isTrue();
            assertThat(tc.contains(node5)).isTrue();
            assertThat(tc.getNodes()).hasSize(3);
        }).anySatisfy(tc -> {
            assertThat(tc.contains(node6)).isTrue();
            assertThat(tc.contains(node8)).isTrue();
            assertThat(tc.getNodes()).hasSize(2);
        }).anySatisfy(tc -> {
            assertThat(tc.contains(node7)).isTrue();
            assertThat(tc.getNodes()).hasSize(1);
        });
